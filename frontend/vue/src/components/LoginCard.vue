  ...
  <v-card-title tag="h2">
    ...
    <v-card-text>
      ...
      <v-form @submit.prevent="loginSubmit">
        ...
        <v-row>
          <v-checkbox label="记住该用户" style="margin-left:20px;" class="d-flex justify-center align-center" :model-value="true"></v-checkbox>
          <a href="#" style="position: absolute;right: 50px;transform: translateY(18px);" class="text-blue text-decoration-none" target="_blank">忘记密码</a>
        </v-row>
        ...
        <v-row>
          <v-col cols="3"></v-col>
          <v-col>
            <v-btn class="mt-2 bg-blue font-weight-bold" type="submit" block style="border-radius: 10px;font-size:18px;">登录</v-btn></v-col
          ><v-col cols="3"></v-col
        ></v-row>

        <v-card-actions>
                <v-spacer></v-spacer>
                还没有账户？<v-btn @click="register" text color="primary">去注册</v-btn>
              </v-card-actions>
